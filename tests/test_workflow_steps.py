...
async def test_summary_step_integration(mock_db_session, mock_session_model):
    ...
    with patch.object(step, 'get_model') as mock_get_model:
        ...
        # Mock database query
        mock_db_session.query = Mock(return_value=Mock(filter=Mock(return_value=Mock(first=Mock(return_value=mock_session_model)))))
...
async def test_execution_status_lifecycle(mock_db_session, clean_registries):
    ...
    # Test QUEUED -> RUNNING
    WorkflowExecutionService.mark_running(execution_id=1, celery_task_id="task_123", db=mock_db_session)
