...
class LLMStep(WorkflowStep):
    ...
    async def _generate(
        ...
        1. Validate inputs
        2. Load session
        3. Validate context requirements are present
        4. Generate messages with context injection
        5. Call LLM
        6. Process response
        ...
        try:
            ...
            # Validate context requirements (flexible - based on step's declared needs)
            missing_keys = [key for key in self.context_requirements if key not in context]
            if missing_keys:
                raise ValueError(
                    f"Missing required context keys for {self.identifier}: {missing_keys}. "
