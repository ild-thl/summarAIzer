...
from app.workflows.execution_context import GenerationState, resolve_target_to_workflow_class, WorkflowRegistry
...
def execute_generated_content(
    ...
    try:
        ...
        created_ids = [
            c.id for c in created_content
            if c.workflow_execution_id == execution_id
        ]
        ...
        logger.info(
            ...
            total_duration_seconds=(datetime.utcnow() - 
                                   ...
                                   ).total_seconds() if content_crud.get_workflow_execution(db, execution_id) else 0,
        )
...
def health_check():
    ...
    logger.info(
        ...
        status="ok",
        worker_name=health_check.request.hostname if hasattr(health_check, 'request') else "unknown",
    )
